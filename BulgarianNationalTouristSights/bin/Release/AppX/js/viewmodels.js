(function () {
    ...
    var commentPlace = function (placeId, text) {
        var promise = new WinJS.Promise(function (success, error) {
            var userInfo = apiClient.users.isUserLoggedIn();
            if (userInfo) {
                var result = getLocation().then(function (pos) {
    ...
    var visitPlace = function (placeId) {
        var promise = new WinJS.Promise(function (succes, error) {
            var userInfo = apiClient.users.isUserLoggedIn();
            if (userInfo) {
                var retult = getLocation().then(function (pos) {
                    var cipher = "";
                    if (!userInfo) {
                        cipher = userInfo.authCode;
                    }
                ...
                error("You need to be logged in to visit places");
            }
        });
        return promise;
    }
